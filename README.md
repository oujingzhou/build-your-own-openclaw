...
```mermaid
graph TB
    subgraph Ch02["Ch.2 CLI 框架"]
        CMD_AGENT["myclaw agent"]
        CMD_GW["myclaw gateway"]
    end

    subgraph Ch03["Ch.3 配置系统"]
        CONFIG["Zod Schema + YAML"]
    end

    subgraph Ch04["Ch.4 网关服务器"]
        GW["Gateway<br/>ws://127.0.0.1:18789"]
    end

    subgraph Ch06["Ch.6-8 通道层"]
        TERM["Terminal"]
        FEISHU["Feishu"]
        TELEGRAM["Telegram"]
    end

    subgraph Ch07["Ch.7 消息路由"]
...
sequenceDiagram
    ...
    participant CH as 通道 (Terminal/飞书/Telegram)
    participant Router as 消息路由器
    participant Agent as Agent Runtime
    participant LLM as LLM (Claude/GPT)
    participant Tool as 工具 (read/exec/...)

    User->>CH: 发送消息
    CH->>Router: RouteRequest {channelId, text, history}
    Router->>Agent: 匹配路由规则 → 选择 Provider
